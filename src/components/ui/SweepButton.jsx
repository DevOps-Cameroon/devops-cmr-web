...
const SweepButton = React.forwardRef(function SweepButton(
  {
    ...
    overlayClassName = "",
    variant = "solid",
    contentClassName = "px-[1.7rem] py-[0.85rem]",
    ...rest
  },
  ref,
) {
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`label-overlay absolute left-0 top-0 flex h-full items-center gap-1.5 whitespace-nowrap text-ink ${contentClassName}`}
        >
          {children}
        </span>
      </span>
      ...
      <span
        className={`label-default relative z-1 flex items-center gap-1.5 whitespace-nowrap ${contentClassName} ${isOutline ? "text-ink" : "text-white"}`}
      >
        {children}
      </span>
    </Tag>
